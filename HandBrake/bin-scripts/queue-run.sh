...
if [ \${DEBUG} = 0 ]; then
##  ${HANDBRAKE} -i ${source} -Z "High Profile"  --non-anamorphic --modulus 2 --keep-display-aspect --no-markers -t ${track} -o ${output} ${subtitle} ${audio} ${chapters} > ${HBCLI_LOG} 2>&1
  ${HANDBRAKE} -i ${source} -Z "HQ 576p25 Surround"  --encoder-level 4.1 --non-anamorphic --modulus 2 --keep-display-aspect --no-markers -t ${track} -o ${output} ${subtitle} ${audio} ${chapters} > ${HBCLI_LOG} 2>&1
fi
